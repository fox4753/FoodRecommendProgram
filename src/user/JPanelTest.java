...
import static javax.swing.JOptionPane.*;
	...
	//음식 정보 패널
	class JPanelRecipe extends JPanel  { 
		...
		int chk = 0;
		  public JPanelRecipe(JPanelTest win){
			  this.win = win;
			  setLayout(null);
			  ...
			  //화면이동
		       ...
		       jButton3 = new JButton("요리 수정");
		       jButton3.setSize(100,40);        
		       jButton3.setLocation(700, 135);
		       add(jButton3);
		       jButton3.addActionListener(new MoveMListListener());
		    ...
		    class MoveMListListener implements ActionListener {         // 버튼 키 눌리면 패널 1번 호출
		        @Override
		        public void actionPerformed(ActionEvent e) {
		        	win.change("panel04");
		        }
	...
	class JPanelRecipeadd extends JPanel{ // 2번째 패널
	    ...
	    public JPanelRecipeadd(JPanelTest win) {
	...
	class JPanelMemberList extends JPanel  { 
	    ...
	    private JTextArea jTextArea1;
		private JPanelTest win;
		private String[] name = {"아이디","이름","이메일","전화번호"};
		private DefaultTableModel dt = new DefaultTableModel(name, 0);
		private JTextArea infotext = new JTextArea();
		String FRecipe = null;
		JTable Foodingrediant;
		int chk = 0;
		  public JPanelMemberList(JPanelTest win){
			  this.win = win;
			  setLayout(null);
			 
			  
			  //음식 재료
			  JTable MemberJT = new JTable(dt){ //요리 재료 목록 테이블 선언
				  		public boolean isCellEditable(int row, int column) { //테이블 셀 수정 불가하게만들기
				  		return false;
				  		}
					  };
			  JScrollPane jsp = new JScrollPane(MemberJT);
			 
			  UserDAO userdao = new UserDAO();	 
			  userdao.MemberList(dt); //멤버 리스트 출력
			  
			  jsp.setSize(600,320);
			  jsp.setLocation(100,40); 
			  add(jsp);
			  

			  //검색	  
			  String[] comboName = {"아이디","이름","이메일","전화번호"};
			  JComboBox combo = new JComboBox(comboName);
			  combo.setSize(70, 24);
			  combo.setLocation(100, 10);
			  add(combo);
			  ...
			  //화면이동
		       ...
		       jButton2.setLocation(700, 90);
		       add(jButton2);
		       jButton2.addActionListener(new MoveTaddListener());
		       ...
		       MemberJT.addMouseListener(new java.awt.event.MouseAdapter() {
		            @Override
		            public void mouseClicked(java.awt.event.MouseEvent evt) {
		                int row = MemberJT.rowAtPoint(evt.getPoint());
		                int col = MemberJT.columnAtPoint(evt.getPoint());
		                String[] Memberinfo = new String[4];
		                for(int i = 0; i< MemberJT.getColumnCount(); i++) {
		                	Memberinfo[i] = (String) MemberJT.getModel().getValueAt(row,i);
		                }
		               MemberEdit ME = new MemberEdit(Memberinfo);
		            }
		       ...
		       Srcbtn.addActionListener(new ActionListener() { //검색 버튼 리스너
					...
					public void actionPerformed(ActionEvent arg0) {
						...
						case "아이디":
							userdao.Search(dt,"username", src); //검색
							break;
						case "이름": 
							userdao.Search(dt,"name", src); //검색
							break;
						case "이메일": 
							userdao.Search(dt,"email", src); //검색
							break;
						case "전화번호": 
							userdao.Search(dt,"phone", src); //검색
							break;
						}
					}
				});	 
		  ...
		  class MoveFsrcListener implements ActionListener {         // 버튼 키 눌리면 패널 1번 호출
		    ...
		    class MoveTaddListener implements ActionListener {         // 버튼 키 눌리면 패널 1번 호출
		        @Override
		        public void actionPerformed(ActionEvent e) {
		        	win.change("panel03");
		        }
	 ...
	 class JPanelFoodList extends JPanel  { 
	    ...
	    private JTextArea jTextArea1;
		private JPanelTest win;
		private String[] name = {"이름","정보","태그"};
		private DefaultTableModel dt = new DefaultTableModel(name, 0);
		private JTextArea infotext = new JTextArea();
		String FRecipe = null;
		JTable Foodingrediant;
		int chk = 0;
		  public JPanelFoodList(JPanelTest win){
			  this.win = win;
			  setLayout(null);
			 
			  
			  //음식 재료
			  JTable FoodJT = new JTable(dt){ //요리 재료 목록 테이블 선언
				  		public boolean isCellEditable(int row, int column) { //테이블 셀 수정 불가하게만들기
				  		return false;
				  		}
					  };
			  JScrollPane jsp = new JScrollPane(FoodJT);
			 
			  DAO dao = new DAO();	 
			  dao.FList(dt); //멤버 리스트 출력
			  
			  FoodJT.getColumn("정보").setWidth(0);
			  FoodJT.getColumn("정보").setMinWidth(0);
			  FoodJT.getColumn("정보").setMaxWidth(0);
			  ...
			  //화면이동
		       ...
		       jButton2.setLocation(700, 90);
		       add(jButton2);
		       jButton2.addActionListener(new MoveMListListener());
		       ...
		       FoodJT.addMouseListener(new java.awt.event.MouseAdapter() {
		            @Override
		            public void mouseClicked(java.awt.event.MouseEvent evt) {
		                int row = FoodJT.rowAtPoint(evt.getPoint());
		                int col = FoodJT.columnAtPoint(evt.getPoint());
		                String[] Foodinfo = new String[4];
		                for(int i = 0; i< FoodJT.getColumnCount(); i++) {
		                	Foodinfo[i] = (String) FoodJT.getModel().getValueAt(row,i);
		                }
		                FoodEdit FE = new FoodEdit(Foodinfo);
		            }
		       ...
		       Srcbtn.addActionListener(new ActionListener() { //검색 버튼 리스너
					...
					public void actionPerformed(ActionEvent arg0) {
						...
						//username name email phone
						case "이름":
							dao.FSrc(dt,"Foodname", src ); //검색
							break;
						case "태그": 
							dao.FSrc(dt,"FoodTag", src ); //검색
							break;
						}
	...
	class JPanelTest extends JFrame{
	    ...
	    public JPanelRecipe jpanel01 = null;
	    public JPanelRecipeadd  jpanel02 = null;
	    public JPanelMemberList jpanel03 = null;
	    public JPanelFoodList jpanel04 = null;
	    ...
	    public void initial() {
		    	...
		    	win.jpanel01 = new JPanelRecipe(win);
		    	win.jpanel02 = new JPanelRecipeadd(win);
		    	win.jpanel03 = new JPanelMemberList(win);
		    	win.jpanel04 = new JPanelFoodList(win);
